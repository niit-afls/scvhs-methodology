...
│   ├── AI_Decision_Log_Template.md    - one Decision Log template, used in every mode
...
**During Validate:** Fill in [AI_Decision_Log_Template.md](scvhs-templates/AI_Decision_Log_Template.md), the same template for every mode, one entry per Constraint and one per Acceptance Criterion. Do not wait until you have read the whole file.
